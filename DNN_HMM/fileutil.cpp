...
//
// $Log: /Audio_Indexing/tools/latspot/latProc/fileutil.cpp $
// 
// 132   11/18/14 2:05p Ganl
// just make compiler happy
// 
// 131   7/22/14 18:28 Fseide
// lattice build() and create() now first write output to a temp $$ file
// and rename at end, to avoid creating corrupt partial files in case they
// fail
// 
// 130   7/11/14 13:43 Fseide
// fseekOrDie() and filesize(FILE*) are now 64-bit capable
// 
// 129   5/14/14 14:40 Fseide
// (removed a 'const' from a scalar by-value parameter)
// 
// 128   4/10/14 11:22 Fseide
// new function GetCurrentExecutableLinkTime() for printing the link-time
// stamp of the current EXE, e.g. for writing a build version tag to the
// log file
// 
// 127   6/07/13 20:37 Fseide
// added experimental generalization of recitifed linear units using an
// optional non-linearity (n-th root) and leakiness
// 
// 126   6/02/13 7:25 Fseide
...
// note: return value different from stdlib's fseek(), which just returns 0 for success or non-0 otherwise
unsigned __int64 fseekOrDie (FILE * f, __int64 offset, int mode)
{
    auto curPos = fgetpos (f);
    if (mode == SEEK_SET)
        fsetpos (f, (unsigned __int64) offset);    // fsetpos() is efficient if seeking forward inside the current read buffer
    else
    {
...
// ----------------------------------------------------------------------------
// moveOrDie(): destructive rename() with error handling
// TODO: a safer implementation is to try rename() first, and if it fails with anything else but 'target exists', then throw, else delete and try again
// ----------------------------------------------------------------------------

#ifndef UNDER_CE // CE only supports Unicode APIs
void moveOrDie (const std::string & from,  const std::string & to)  { _unlink (to.c_str());  renameOrDie (from, to); }
#endif

void moveOrDie (const std::wstring & from, const std::wstring & to) { _wunlink (to.c_str()); renameOrDie (from, to); }
...
void fputfloat (HANDLE f, float v)
{
    fwriteOrDie (&v, sizeof (v), 1, f);
}
...
// ----------------------------------------------------------------------------
// GetCurrentExecutableLinkTime() -- get current executable's link time from PE header
// ----------------------------------------------------------------------------

static const wchar_t * getcurrentexepath() { wchar_t * path = nullptr; _get_wpgmptr (&path); return path; } // path of current process' executable, i.e. ourselves
time_t GetCurrentExecutableLinkTime()
{
    auto_file_ptr f = fopenOrDie (getcurrentexepath(), L"rb");
    for (size_t k = 0; k < 128 && !feof (f); k++)   // scan for PE header; we find it at byte 232; it's 60 in http://stackoverflow.com/questions/1600962/displaying-the-build-date
        if (fgetint (f) == IMAGE_NT_SIGNATURE)
            return fgetint (f)/*skip 2 WORD fields*/, fgetint (f);  // link time stamp in seconds since 1970 (time_t)
    return -1;  // failed (ctime_s() will fail on this value)
}
std::string ctime (time_t t) { char buf[26]; if (ctime_s (buf, _countof (buf), &t) == 0) { buf[strlen(buf)-1] = 0/*strip silly newline*/; return buf; } else return "(invalid)"; }
