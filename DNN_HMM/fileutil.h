...
// fail
// 
// 77    7/17/14 18:00 Fseide
// now saving lattices to a temp file first, to avoid corrupt files in
// case there is a write error
// 
// 76    7/17/14 17:55 Fseide
// new helper function safesave()
// 
// 75    7/11/14 13:43 Fseide
// fseekOrDie() and filesize(FILE*) are now 64-bit capable
// 
// 74    4/10/14 11:22 Fseide
...
unsigned __int64 fseekOrDie (FILE * f, __int64 offset, int mode = SEEK_SET);
unsigned __int64 fgetpos (FILE * f);
#define ftellOrDie fgetpos
...
void moveOrDie (const std::string & from, const std::string & to);
void moveOrDie (const std::wstring & from, const std::wstring & to);
...
void fputfloat (HANDLE, float val);
...
// ----------------------------------------------------------------------------
// safesave() -- helper to save a file safely
// e.g. safesave ("out.lat", [&] (const char * path) { plat->write (path); });
// ----------------------------------------------------------------------------

template<typename SAVEFUNCTION>
static inline void safesave (const char * fname, SAVEFUNCTION & f)
{
    std::string temppath (fname);
    // we first save to a temp name based on the original name
    temppath.append ("$$");
    try
    {
        f (temppath.c_str());       // or throw
        // success: rename temp to the target path
        _unlink (fname);            // without error check since most of the time the file won't exist yet
        renameOrDie (temppath, fname);
        // we are hosed if the rename fails, but that's unlikely, e.g. for access permissions, we would have failed before
    }
    catch (...)                     // error: delete temp file and pass on exception
    {
        _unlink (temppath.c_str());
        throw;
    }
}

// ----------------------------------------------------------------------------
// GetCurrentExecutableLinkTime() -- get an executable's link time
// ----------------------------------------------------------------------------

time_t GetCurrentExecutableLinkTime();
std::string ctime (time_t t);   // and helper to print it
