...
#include "f0.h"
#include <math.h>
static void get_cand(register Cross *cross,
                     register float *peak,
                     register int *loc,
                     register int nlags,
                     register int *ncand,
                     register float cand_thresh);
static void peak(register float *y,
                 register float *xp,
                 register float *yp);
static void do_ffir(register float        *buf,
                    ...
                    register int skip,
                    register int init);
static int lc_lin_fir(register float fc,
                      register int *nf,
                      register float *coef);
static int downsamp(register float *in, 
                    register float *out, 
                    register int samples, 
                    register int *outsamps, 
                    register int state_idx, 
                    register int decimate, 
                    register int ncoef, 
                    register float fc[], 
                    register int init);

/* ----------------------------------------------------------------------- */
void get_fast_cands(register float *fdata, 
                    register float *fdsdata, 
                    register int ind, 
                    register int step, 
                    register int size, 
                    register int dec, 
...
__declspec(thread) static float *foutputResource = NULL;
__declspec(thread) static float *coResource = NULL, *memResource = NULL;
__declspec(thread) static int flagFoutputResource = -1, flagCoResource = -1, flagmMemResource = -1;
__declspec(thread) static int first_time = 1;

void Free_Get_Cands()
{
	if (flagFoutputResource > 0)
	{
		memoryDelete foutputResource;
		foutputResource = NULL;
		flagFoutputResource = -1;
	}
	if (flagCoResource > 0)
	{
		memoryDelete coResource;
		coResource = NULL;
		flagCoResource = -1;
	}
	if (flagmMemResource > 0)
	{
		memoryDelete memResource;
		memResource = NULL;
		flagmMemResource = -1;
	}
...
/* ----------------------------------------------------------------------- */
float *downsample(register float *input,
                  register int samsin,
                  register int state_idx,
                  register double freq,
                  register int *samsout,
                  register int decimate, 
                  register int first_time, 
                  register int last_time)
{
  __declspec(thread) static float b[2048];
  __declspec(thread) static float *foutput;
  float        beta = 0.0;
  __declspec(thread) static int  ncoeff = 127, ncoefft = 0;
  int init;
  ...
  if(input && (samsin > 0) && (decimate > 0) && *samsout) {
    ...
    if(first_time){
      ...
      foutput = memoryNew float[nbuff];
	  foutputResource = foutput;
	  flagFoutputResource = 1;
                    ...
                    register int init)
{
  ...
  __declspec(thread) static float *co = NULL, *mem = NULL;
  __declspec(thread) static float state[1000];
  __declspec(thread) static int fsize = 0, resid = 0;
  register int i, j, k, l;
  register float *sp;
  register float *buf1;

  buf1 = buf;
  if ((ncoef > fsize) || (first_time ==1)){/*allocate memory for full coeff. array and filter memory */
	  if (coResource != NULL)
	  {
		  memoryDelete co;
		  co = NULL;
	  ...
	  fsize = 0;
	  i = (ncoef + 1) * 2;
	  if (!((co = memoryNew float[i]) &&
		  (mem = memoryNew float[i]))) {
		  coResource = NULL;
		  memResource = NULL;
		  flagCoResource = -1;
		  flagmMemResource = -1;
...
static void peak(register float *y,   /* vector of length 3 defining peak */
                 register float *xp,  /* x,y values of parabolic peak fitting the input points. */
                 register float *yp)
{
  register float a, c;
