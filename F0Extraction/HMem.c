...
__declspec( thread ) int debug_level = 0;               /* For esps linking */
...
__declspec(thread) MemHeap gstack;   /* global MSTAK for general purpose use */
...
__declspec(thread) static MemHeapRec *heapList = NULL;
...
void UnInitMem_gstack()
{
	DeleteHeap(&gstack);
}

/*This is free for InitMem(void)*/
void UnInitMem()
{
	// Change a value, otherwise don't let to delete it
	gcheap.type = MSTAK;
	DeleteHeap(&gcheap);
}
