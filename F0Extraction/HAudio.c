...
#include <Alib.h>
__declspec( thread )  static Audio *audio_dev=NULL;
__declspec( thread )  static int audio_cnt=0;
#endif
#ifdef OSS_AUDIO
#include <unistd.h>
#include <fcntl.h>
...
__declspec( thread )  static short zero_buf[BUF_SIZE];
__declspec( thread )  static int audio_fd;
__declspec( thread )  static int mixer_fd;
__declspec( thread )  static unsigned short audio_io = 0x00;
__declspec( thread )  static int frag_size;
__declspec( thread ) static audio_buf_info audio_info;
#endif
...
__declspec(thread) static VolType volType = v_peak;
...
__declspec(thread) static volatile Boolean stopSignalled;
...
__declspec(thread) static DWORD sMagic=-1;
...
__declspec( thread ) static AudioIn sAudioIn=NULL;
...
__declspec(thread) static AudioOut sAudioOut=NULL;
...
__declspec(thread) static AudioIn sigAudio;  /* Globals used for signalling */
__declspec(thread) static int sigNum = NULLSIG;
__declspec(thread) static volatile Boolean waitForSigH;
__declspec(thread) static volatile Boolean alreadyFilling;
__declspec(thread) static int nz = 0;
