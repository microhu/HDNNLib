...
__declspec(thread) static MemHeap sigpHeap;
static ConfParam *cParm[MAXGLOBS];       /* config parameters */
static int numParm = 0;

__declspec(thread) static int hamWinSize = 0;          /* Size of current Hamming window */
__declspec(thread) static Vector hamWin = NULL;        /* Current Hamming window */

__declspec(thread) static int cepWinSize = 0;            /* Size of current cepstral weight window */
__declspec(thread) static int cepWinL = 0;               /* Current liftering coeff */
__declspec(thread) static Vector cepWin = NULL;        /* Current cepstral weight window */
...
void UnInitSigP(void)
{

}

void InitSigPHeap(void)
{
    CreateHeap(&sigpHeap, "sigpHeap", MSTAK, 1, 0.0, 5000, 5000);
}

void UnInitSigPHeap(void)
{
    hamWinSize = 0;
    hamWin = NULL;
    cepWinSize = 0;
