...
__declspec( thread ) static Frame *headF = NULL, *tailF = NULL, *cmpthF = NULL;

__declspec( thread ) static  int *pcands = NULL;        /* array for backtracking in convergence check */
__declspec( thread ) static int cir_buff_growth_count = 0;

__declspec( thread ) static int size_cir_buffer,        /* # of frames in circular DP buffer */
 ...
 * DP parameters
 */
__declspec( thread ) static float tcost, tfact_a, tfact_s, frame_int, vbias, fdouble, wdur, ln2,
             freqwt, lagwt;
__declspec( thread ) static int step, size, nlags, start, stop, ncomp, *locs = NULL;
__declspec( thread ) static short maxpeaks;

__declspec( thread ) static int wReuse = 0;  /* number of windows seen before resued */
__declspec( thread ) static Windstat *windstat;

__declspec( thread ) static float *f0p = NULL, *vuvp = NULL, *rms_speech = NULL,
             *acpkp = NULL, *peaks = NULL;
__declspec( thread ) static int first_time = 1, pad;


/*--------------------------------------------------------------------*/
int get_Nframes(register long buffsize, 
                register int pad, 
                register int step)
{
  if (buffsize < pad)
...
/*--------------------------------------------------------------------*/
int init_dp_f0(register int freq, 
               register F0_params *par, 
               register long *buffsize, 
               register long *sdstep)
{
  int nframes;
  int i;
 ...
 */
  ...
  step = (int)(frame_int * freq + 0.5);
  size = (int)(par->wind_dur * freq + 0.5);
  frame_int = (float)step/freq;
  wdur = (float)size/freq;
  start = (int)(freq / par->max_f0 + 0.5);
  stop = (int)(freq / par->min_f0 + 0.5);
  nlags = stop - start + 1;
...
/*--------------------------------------------------------------------*/
int dp_f0(register float *fdata, 
          register int buff_size, 
          register int sdstep, 
          register int freq,
          register F0_params *par,          /* analysis control parameters */
          register float **f0p_pt, 
          register float **vuvp_pt, 
          register float **rms_speech_pt, 
          register float **acpkp_pt, 
          ...
          register int last_time)
{
  ...
  Stat *stat = NULL;
  Stat* get_stationarity(float*, int, int, int, int, int);
  ...
  for(i = 0; i < nframes; i++) {
    ...
    for(k=0; k<ncand; k++){        /* for each of the current candidates... */
      ...
      if((loc2 = headF->dp->locs[k]) > 0) { /* current cand. is voiced */
        for(j=0; j<ncandp; j++){ /* for each PREVIOUS candidate... */
          ...
          if (loc1 > 0) { /* prev. was voiced */
            ...
            ferr = ttemp * freqwt;
#if _DEBUG
           // printf("VV: i=%d j=%d k=%d Es=%f dpvals=%f Ep=%f\n", i,j,k,headF->dp->mpvals[k],headF->prev->dp->dpvals[j],ferr);
#endif
          } else {                /* prev. was unvoiced */
            ferr = tcost + (tfact_s * sta[i]) + (tfact_a / rms_ratio[i]);
#if _DEBUG
         //   printf("UV: i=%d j=%d k=%d Es=%f dpvals=%f Et=%f\n", i,j,k,headF->dp->mpvals[k],headF->prev->dp->dpvals[j],ferr);
#endif
          }
        ...
        for(j=0; j<ncandp; j++){ /* for each PREVIOUS candidate... */
          ...
          if (headF->prev->dp->locs[j] > 0) { /* previous was voiced */
            ferr = tcost + (tfact_s * sta[i]) + (tfact_a * rms_ratio[i]);
#if _DEBUG
         //   printf("VU: i=%d j=%d k=%d Es=%f dpvals=%f Et=%f\n", i,j,k,headF->dp->mpvals[k],headF->prev->dp->dpvals[j],ferr);
#endif
          }
          else {
            ferr = 0.0;
#if _DEBUG
       //     printf("VU: i=%d j=%d k=%d Es=%f dpvals=%f Ep=%f\n", i,j,k,headF->dp->mpvals[k],headF->prev->dp->dpvals[j],ferr);
#endif
          }
          /*    Add in cumulative cost associated with previous peak. */
          err = ferr + headF->prev->dp->dpvals[j];
      ...
      if (first_time && i==0) {                /* this is the first frame */
        ...
        headF->dp->prept[k] = 0;
#if _DEBUG
        //printf("%d %d %d %f %f\n", i, k, headF->dp->prept[k], headF->dp->dpvals[k], 0);
#endif
      } else {
        headF->dp->dpvals[k] = errmin + headF->dp->mpvals[k];
        headF->dp->prept[k] = minloc;
#if _DEBUG
        //printf("%d %d %d %f %f\n", i, k, headF->dp->prept[k], headF->dp->dpvals[k], errmin);
#endif
      }
  ...
  if( num_active_frames >= size_frame_hist  || last_time ){
    ...
    while( frm != tailF->prev && checkpath_done){
      if( i == output_buf_size ){ /* Need more room for outputs? */
        ...
        if(rms_speech) memoryDelete rms_speech;
        rms_speech = memoryNew float[output_buf_size];
        if(f0p) memoryDelete f0p;
        f0p = memoryNew float[output_buf_size];
        if(vuvp) memoryDelete vuvp;
        vuvp = memoryNew float[output_buf_size];
        if(acpkp) memoryDelete acpkp;
        acpkp = memoryNew float[output_buf_size];
      }
...
/*--------------------------------------------------------------------*/
Frame * alloc_frame(register int nlags, register int ncands)
{
  Frame *frm;
  int j;

  frm = memoryNew Frame;
  frm->dp = memoryNew Dprec;
  frm->dp->ncands = 0;
  frm->cp = memoryNew Cross;
  frm->cp->correl = memoryNew float[nlags];
  /* Allocate space for candidates and working arrays. */
  frm->dp->locs = memoryNew short[ncands];
  frm->dp->pvals = memoryNew float[ncands];
  frm->dp->mpvals = memoryNew float[ncands];
  frm->dp->prept = memoryNew short[ncands];
  frm->dp->dpvals = memoryNew float[ncands];
...
static int save_windstat(register float *rho, 
                         register int order, 
                         register float err, 
                         register float rms)
{
...
/*--------------------------------------------------------------------*/
static int retrieve_windstat(register float *rho, 
                             register int order, 
                             register float *err, 
                             register float *rms)
{
...
/*--------------------------------------------------------------------*/
static float get_similarity(register int order, 
                            register int size, 
                            register float *pdata, 
                            register float *cdata,
                            register float *rmsa, 
                            register float *rms_ratio, 
                            register float pre, 
                            register float stab, 
                            register int w_type, 
                            register int init)
{
  ...
  float itakura(int,float*,float*,float*,float*), wind_energy(float*,int,int);
...
/* (In the lpc() calls below, size-1 is used, since the windowing and
  ...
  if(!init) {
      ...
      t = (float)(itakura(order,b,&b0,rho1+1,&err1) - .8);
      if(rms1 > 0.0)
...
__declspec( thread ) static Stat *stat = NULL;
__declspec( thread ) static float *mem = NULL;
Stat* get_stationarity(register float *fdata, 
                              register int freq, 
                              register int buff_size, 
                              register int nframes, 
                              register int frame_step, 
                              register int first_time)
{
  __declspec(thread) static int nframes_old = 0, memsize;
  ...
  if( nframes_old < nframes || !stat || first_time){
    ...
    nframes_old = nframes;
    if(stat){ 
      memoryDelete stat->stat;
      memoryDelete stat->rms;
      memoryDelete stat->rms_ratio;
      memoryDelete stat;
    }
    stat = memoryNew Stat[nframes];
    stat->stat = memoryNew float[nframes];
    stat->rms = memoryNew float[nframes];
    stat->rms_ratio = memoryNew float[nframes];
    memsize = (int) (STAT_WSIZE * freq) + (int) (STAT_AINT * freq);
    mem = memoryNew float[memsize];
  ...
  if((order = 2 + (int)(freq/1000.0)) > BIGSORD) {
    Fprintf(stderr,
...
void free_dp_f0()
{
  ...
  for(i = 0; i < size_cir_buffer; i++) {
    next = frm->next;
    memoryDelete frm->cp->correl;
    memoryDelete frm->dp->locs;
    memoryDelete frm->dp->pvals;
    memoryDelete frm->dp->mpvals;
    memoryDelete frm->dp->prept;
    memoryDelete frm->dp->dpvals;
    memoryDelete frm->cp;
    memoryDelete frm->dp;
    memoryDelete frm;
    frm = next;
