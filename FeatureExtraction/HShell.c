...
__declspec(thread) union Byte4Union
{
   int intvalue;
...
__declspec(thread) union Byte2Union
{
   short shortvalue;
   byte bytearray[sizeof(short) / sizeof(byte)];
}Byte2U;

__declspec(thread) static ExtFile extFiles[MAXEFS];        /* circ buf of ext file names */
__declspec(thread) static int extFileNext = 0;             /* next slot to save into */
__declspec(thread) static int extFileUsed = 0;             /* total ext files in buffer */
...
__declspec(thread) static VersionEntry *vInfoHd = NULL;  /* head of version info list */
__declspec(thread) static VersionEntry *vInfoTl = NULL;  /* tail of version info list */
...
static ReturnStatus ReadConfigFile(char *fname)
{
   ...
   __declspec(thread) static int recurse = 0;
...
__declspec(thread) static int nextarg = 1;         /* next arg to return in GetxxxArg */
static char *defargs[2] = { "<Uninitialised>", "" };
static char **arglist = defargs;/* actual arg list */
__declspec(thread) static FILE *script = NULL;   /* script file if any */
__declspec(thread) static int scriptcount = 0;   /* num words in script */
__declspec(thread) static char scriptBuf[256];   /* buffer for current script arg */
static Boolean scriptBufLoaded = FALSE;
__declspec(thread) static Boolean wasQuoted;     /* true if next arg was quoted */
...
__declspec(thread) static int maxTry = 1;
...
{
   __declspec(thread) static char stat[MAXSTRLEN * 4];
   Boolean noSing,noDbl;
   int n;
   unsigned char *p,*d;
...
/* EXPORT->WriteShortToByteArray: write n shorts to bp */
void WriteShortToByteArray(byte *bp, int *pos, short *s, int n)
{
   int j, v;
   short *p;
   if (vaxOrder && !natWriteOrder){
   ...
   for (j = 0; j < n; j++)
   {
      Byte2U.shortvalue = s[j];
      for (v = 0; v < sizeof(short) / sizeof(byte); v++)
      {
         bp[(*pos)++] = Byte2U.bytearray[v];
      }
   }
...
/* EXPORT->WriteInt: write n ints to bp */
void WriteIntToByteArray(byte *bp, int *pos, int *i, int n)
{
   int j, v;
   int *p;
   if (vaxOrder && !natWriteOrder){
...
/* EXPORT->WriteFloatToByteArray: write n floats to bp */
void WriteFloatToByteArray(byte*bp, int *pos, float *x, int n)
{
   int j, v;
   float *p;
   if (vaxOrder && !natWriteOrder){
   ...
   for (j = 0; j < n; j++)
   {
      Byte4U.floatvalue = x[j];
      for (v = 0; v < sizeof(float) / sizeof(byte); v++)
      {
         bp[(*pos)++] = Byte4U.bytearray[v];
      }
   }
...
int RestartHShell(void)
{
   nextarg = 1;
   scriptcount = 0;
   infoPrinted = FALSE;      /* set when -A -B or -V is used */
   abortOnError = FALSE;     /* causes HError to abort */
   printVersionInfo = FALSE; /* request version info */
   showConfig = FALSE;       /* show configuration params */
   noNumEscapes = FALSE;     /* Prevent writing in \012 format */
   natReadOrder = FALSE;     /* Preserve natural mach read order*/
   natWriteOrder = FALSE;    /* Preserve natural mach write order*/
   extendedFileNames = TRUE; /* allow extended file names */
   vaxOrder = FALSE;
   extFileNext = 0;             /* next slot to save into */
   extFileUsed = 0;
...
//FreeConfig(confList);
void DeleteConfigEntryObject(ConfigEntry *object)
{
	ConfigEntry *current;
	ConfigEntry *head;
	...
	{
		head = current->next;
		free(current->param.name);
		free(current->param.user);
		if (current->param.kind == StrCKind)
		{
			free(current->param.val.s);
		}
		free(current);
		current = head;
	}
...
void ResetShell(void)
{
    ...
    {
        //============
        DeleteConfigEntryObject(confList);
        confList = NULL;
    }
    ...
    //============
    free(savedCommandLine);
    savedCommandLine = NULL;
    return;
}
