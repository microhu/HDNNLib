...
__declspec(thread) static int trace = 0;
...
__declspec(thread) union ByteLongUnion
{
   long longvalue;
   byte bytevalue[sizeof(long) / sizeof(byte)];
}ByteLU;

__declspec(thread) union ByteShortUnion
{
   short shortvalue;
   byte bytevalue[sizeof(short) / sizeof(byte)];
}ByteSU;

__declspec(thread) union ByteIntUnion
{
   int32 intvalue;
   byte bytevalue[sizeof(int) / sizeof(byte)];
}ByteIU;
...
__declspec(thread) static int cNIST;    /* current input char */
__declspec(thread) static int cCount;   /* num bytes read */
...
__declspec(thread) static FieldList  ESIGFieldList;
...
void WriteHTKHeaderToArray(byte *mfcdata,int *pos, long nSamp, long sampP, short sampS, 
                    short kind, Boolean *bSwap)
{
   HTKhdr hdr;
   int n = sizeof hdr;
   ...
   ByteIU.intvalue=hdr.nSamples;
   for( j=0;j<sizeof(int)/sizeof(byte);j++)
   {
	   mfcdata[(*pos)++]=ByteIU.bytevalue[j];
   }

   ByteIU.intvalue=hdr.sampPeriod;
   for(j=0;j<sizeof(int)/sizeof(byte);j++)
   {
	   mfcdata[(*pos)++]=ByteIU.bytevalue[j];
   }
  
   ByteSU.shortvalue=hdr.sampSize;
   for( j=0;j<sizeof(short)/sizeof(byte);j++)
   {
	   mfcdata[(*pos)++]=ByteSU.bytevalue[j];
   }
   ByteSU.shortvalue=hdr.sampKind;
   for(j=0;j<sizeof(short)/sizeof(byte);j++)
   {
	   mfcdata[(*pos)++]=ByteSU.bytevalue[j];
   }
...
/* EXPORT-> OpenWaveInputFromMemory: open input waveform file from memory*/
Wave OpenWaveInputFromMemory(MemHeap *x, byte* inputdata, int ilen, FileFormat fmt, HTime winDur,
                             HTime frPeriod, HTime *sampPeriod)
{
   Wave w;                /* New Wave Object */
   InputAction ia = (InputAction)0;      /* flags to enable conversions etc */
   ...
   for (i = 0; i < 4; i++) str[i] = inputdata[pos++];
   if (strncmp(str, "RIFF", 4) != 0) HError(1000, "the input file is not RIFF format!\n"); //riff
   pos += 4;																		  // chunk size
   for (i = 0; i < 4; i++) str[i] = inputdata[pos++];										 // wave
   if (strncmp(str, "WAVE", 4) != 0) HError(1000, "the input file is not WAVE format!\n");

   while (1){
      if (pos>ilen){
         HRError(6251, "No data portion in WAVE file"); break;
      }
      for (i = 0; i < 4; i++) str[i] = inputdata[pos++];
      ByteIU.intvalue = 0;
      for (i = 0; i < 4; i++) ByteIU.bytevalue[i] = inputdata[pos++]; //number of channels
      lens = ByteIU.intvalue;
      if (ia & DoBSWAP) SwapInt32(&lens);

      if (strncmp("data", str, 4) == 0) break;
      if (strncmp("fmt ", str, 4) == 0){
         ByteSU.shortvalue = 0;
         for (i = 0; i < 2; i++) ByteSU.bytevalue[i] = inputdata[pos++]; //wav type
         type = ByteSU.shortvalue;
         if (ia & DoBSWAP) SwapShort(&type);
         ...
         ByteSU.shortvalue = 0;
         for (i = 0; i < 2; i++) ByteSU.bytevalue[i] = inputdata[pos++]; //number of channels
         chans = ByteSU.shortvalue;
         if (ia & DoBSWAP) SwapShort(&chans);
         if (chans != 1 && chans != 2){
            HRError(6251, "Neither mono nor stereo!");
            break;
         ...
         ByteIU.intvalue = 0;
         for (i = 0; i < 4; i++) ByteIU.bytevalue[i] = inputdata[pos++]; // samplerate
         samprate = ByteIU.intvalue;
         if (ia & DoBSWAP) SwapInt32(&samprate);
         w->sampPeriod = 1.0E7 / (float)samprate;

         ByteIU.intvalue = 0;
         for (i = 0; i < 4; i++) ByteIU.bytevalue[i] = inputdata[pos++]; //byterate
         ByteSU.shortvalue = 0;
         for (i = 0; i < 2; i++) ByteSU.bytevalue[i] = inputdata[pos++]; //block align
         // pos+=6;   // byterate + bock align

         ByteSU.shortvalue = 0;
         for (i = 0; i < 2; i++) ByteSU.bytevalue[i] = inputdata[pos++]; // bitspersample
         samplesize = ByteSU.shortvalue;
         if (ia & DoBSWAP) SwapShort(&samplesize);

         if (samplesize != 16 && samplesize != 8){
            HRError(6251, "Only 8/16 bit audio supported"); break;
         }
         if ((type == WAVE_FORMAT_MULAW || type == WAVE_FORMAT_ALAW) && samplesize != 8){
            HRError(6251, "Only 8-bit mu-law/a-law supported");
            break;
         }
         if (type == WAVE_FORMAT_PCM && samplesize == 8)
            ia = (InputAction)(ia | (Do8_16 | DoCVT));
   ...
   w->nSamples = lens * 8 / chans / samplesize;
   w->hdrSize = pos;

   w->nAvail = w->nSamples;
   w->data = (short *)New(w->mem, w->nSamples * 2);

   for (i = 0; i < w->nAvail; i++){
      ByteSU.shortvalue = 0;
      for (j = 0; j<2; j++) ByteSU.bytevalue[j] = inputdata[pos++];
      w->data[i] = ByteSU.shortvalue;
      //	if (chans==2) pos+=4; // for stero not debugged yet
   }
   ...
   *sampPeriod = w->sampPeriod;
   ...
   if (ia&DoCVT)
      ...
      case NIST:
         if (ConvertNISTData(w, ia) < SUCCESS){
            //   FClose(f,w->isPipe);  
            HRError(6270, "OpenWaveInputFromMemory: No Convertor for format %s", fmtmap[w->fmt]);
            return(NULL);
         }
         break;
      case SUNAU8:
...
Wave OpenWaveInputFromMemory_old(MemHeap *x, byte* inputdata, int ilen, FileFormat fmt, HTime winDur,
                                 HTime frPeriod, HTime *sampPeriod)
{
   Wave w;                /* New Wave Object */
   InputAction ia = (InputAction)0;      /* flags to enable conversions etc */
   ...
   for (i = 0; i < 4; i++) str[i] = inputdata[pos++];
   if (strcmp(str, "RIFF") != 0) HError(1000, "the input file is not RIFF format!\n"); //riff
   pos += 4;																		  // chunk size
   for (i = 0; i < 4; i++) str[i] = inputdata[pos++];										 // wave
   if (strcmp(str, "WAVE") != 0) HError(1000, "the input file is not WAVE format!\n");
   pos += 10;				//subchunk1id subchunk1size audioformat
   ByteSU.shortvalue = 0;
   for (i = 0; i < 2; i++) ByteSU.bytevalue[i] = inputdata[pos++]; //number of channels
   chans = ByteSU.shortvalue;
   ...
   ByteIU.intvalue = 0;
   for (i = 0; i < 4; i++) ByteIU.bytevalue[i] = inputdata[pos++]; // samplerate
   w->sampPeriod = 1.0E7 / (float)ByteIU.intvalue;
   ...
   pos += 4;   //"data"
   ByteIU.intvalue = 0;
   for (i = 0; i < 4; i++) ByteIU.bytevalue[i] = inputdata[pos++]; // subchunk2size
   w->nSamples = ByteIU.intvalue * 8 / chans / samplesize;
   if (ilen - pos < ByteIU.intvalue) HError(1000, "the length of input wav file is short than expected in the wav file\n");
   w->hdrSize = pos;
...
void RestartHWave()
{
   natReadOrder = FALSE;    /* Preserve natural read byte order*/
   natWriteOrder = FALSE;
   numParm = 0;
}
/* --------------------------------  HWave.c ------------------------------- */
